...
const int PIPE_Y_SPEED = 1;
const int PIPE_Y_RANGE = 100;
...
void renderGameOver() {
    ...
    SDL_Texture* gameOverTexture = createTextTexture("Game Over! P to play agian or Q to quit", textColor);
    int textWidth, textHeight;
...
void updateGame(Bird& bird, Pipe pipes[], int& numPipes, bool& gameOver) {
    ...
    for (int i = 0; i < numPipes; i++) {
        ...
        if (pipes[i].y < 0 || pipes[i].y > SCREEN_HEIGHT - PIPE_GAP) {
            pipes[i].yDirection *= -1; // Đảo ngược hướng di chuyển của ống
        }

        // Nếu ống đi ra khỏi màn hình, đặt lại vị trí ống và tăng điểm
        if (pipes[i].x < -PIPE_WIDTH) {
            pipes[i].x = SCREEN_WIDTH; // Đặt lại ống ở phía bên phải màn hình
            // Đặt lại vị trí ngẫu nhiên của ống
